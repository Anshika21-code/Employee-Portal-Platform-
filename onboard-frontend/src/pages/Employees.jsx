...
// // ─────────────────────────────────────────────────────────────
// //  TaskPanel — FIXED version (paste this over your TaskPanel function)
// // ─────────────────────────────────────────────────────────────
// function TaskPanel({ employeeId, onProgressUpdate }) {
//   const [tasks, setTasks]       = useState([]);
//   const [loading, setLoading]   = useState(true);
...
//   // ✅ ALL useEffects FIRST — before any if/return statements
//   useEffect(() => {
//     fetch(`http://127.0.0.1:5000/api/tasks/employee/${employeeId}`)
//       .then(res => res.json())
//       .then(data => {
...
//                   disabled={isUpdating}
//                   onChange={(e) => handleStatusChange(task.id, e.target.value)}
//                   className="appearance-none pl-7 pr-6 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white cursor-pointer disabled:cursor-wait"
//                 >
//                   <option value="Not Started">Not Started</option>
//                   <option value="In Progress">In Progress</option>
//                   <option value="Completed">Completed</option>
...
const taskStatusStyle = {
  "Not Started": "bg-gray-100 text-muted-foreground",
  "In Progress":  "bg-blue-100 text-blue-700",
  "Completed":    "bg-emerald-100 text-emerald-700",
};
            ...
            >
              ...
              <div className="relative inline-flex items-center">
                ...
                <select
                  ...
                  disabled={isUpdating}
                  onChange={(e) => handleStatusChange(task.id, e.target.value)}
                  className="appearance-none pl-7 pr-6 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-card cursor-pointer disabled:cursor-wait"
                >
                  <option value="Not Started">Not Started</option>
                  <option value="In Progress">In Progress</option>
                  <option value="Completed">Completed</option>
    ...
    <div className="min-h-screen bg-background p-6 font-sans">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">{employees.length} total employees in onboarding</p>
        </div>
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-xl shadow-sm transition-colors">
          <UserPlus size={15} />
          Add Employee
      ...
      {showForm && (
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6 mb-6">
          ...
          {step === 2 && (
            <div>
              <p className="text-sm text-muted-foreground mb-4">Add tasks for the new employee.</p>
              <div className="space-y-3 mb-4">
                {tasks.map((task, index) => (
                  <div key={index} className="grid grid-cols-12 gap-2 items-start bg-gray-50 p-3 rounded-xl">
                    <input type="text" placeholder="Task title *" value={task.title} onChange={(e) => updateTask(index, "title", e.target.value)} className="col-span-3 border border-gray-200 p-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-card" />
                    <input type="text" placeholder="Description" value={task.description} onChange={(e) => updateTask(index, "description", e.target.value)} className="col-span-5 border border-gray-200 p-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-card" />
                    <div className="relative col-span-3">
                      <label className="absolute -top-2 left-3 bg-card px-1 text-xs text-gray-400">Due Date</label>
                      <input type="date" value={task.due_date} onChange={(e) => updateTask(index, "due_date", e.target.value)} className="w-full border border-gray-200 p-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-card" />
                    </div>
                    <button onClick={() => removeTaskRow(index)} className="col-span-1 text-red-400 hover:text-red-600 text-lg font-bold flex items-center justify-center pt-1">×</button>
                  </div>
                ))}
      ...
      {/* Filters */}
      <div className="bg-card rounded-2xl shadow-sm border border-border p-4 mb-6 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input type="text" placeholder="Search by name, role, or department..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-transparent" />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((emp, i) => {
          const cfg        = statusConfig[emp.status] || statusConfig["on-track"];
          const StatusIcon = cfg.icon;
          const isExpanded = expandedId === emp.id;  //  inside .map()

          return (
            <div key={emp.id} className="relative bg-card rounded-2xl shadow-sm border border-border p-5 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-border">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => toggleExpand(emp.id)}
                    className="flex items-center gap-1 text-xs text-muted-foreground hover:text-gray-700 font-medium transition-colors px-2 py-1 rounded-lg hover:bg-gray-100"
                  >
                    {isExpanded ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
                    {isExpanded ? "Hide" : "Tasks"}
                  </button>
